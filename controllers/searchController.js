...
=======
      ...
      const searchUrl = `${apiBaseUrl}/api/v1/pdf/highlight/${encodeURIComponent(
        searchTerm
      )}`;
      console.log("Attempting API call to:", searchUrl);
>>>>>>> Stashed changes
  ...
  searchDocuments: async (request, reply) => {
    ...
    if (!documentName || !searchTerm) {
      return reply
        .code(400)
        .send({ error: "Document name and search term are required" });
    }
...
=======
        ...
        const documentUrl = new URL(
          `/documents/${encodedDocName}/search/${encodedSearchTerm}`,
          baseUrl
        ).toString();
...
=======
      ...
      const apiUrl = `${baseUrl}/search1Highligth/${encodeURIComponent(
        searchTerm
      )}`;
>>>>>>> Stashed changes
